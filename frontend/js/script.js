...
async function showRequestBookModal() {
    try {
        ...
        const options = books.map(b => `<option value="${b.id}">${b.title} — ${b.author} (${b.status})</option>`).join('');

        const formHTML = `
            <form id="requestForm">
                <div class="form-group">
                    <label for="requestBook">Select Book *</label>
                    <select id="requestBook" name="book_id" required>
        ...
        if (data.success) {
            ...
            if (data.journals && data.journals.length > 0) {
                ...
                if (yearFilter) {
                    ...
                    const years = [...new Set(data.journals
                        .filter(j => j.year)
                        .map(j => j.year)
...
async function handleUploadJournal() {
    ...
    try {
        ...
        const res = await fetch(`${API_BASE_URL}/journals.php`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${authToken}` },
            body: fd
        });
        const data = await res.json();
        if (data.success) {
            msg.style.display = 'block'; msg.style.backgroundColor = '#d4edda'; msg.textContent = 'Upload successful';
            await loadJournals();
            setTimeout(hideUploadJournalModal, 1200);
        } else {
            msg.style.display = 'block'; msg.style.backgroundColor = '#f8d7da'; msg.textContent = data.message || 'Upload failed';
        }
    } catch (error) {
        console.error('Upload error:', error);
        msg.style.display = 'block'; msg.style.backgroundColor = '#f8d7da'; msg.textContent = 'Network error during upload';
    } finally {
        const btn = document.getElementById('submitUploadJournal');
